...
//后台回调
function ue_callback(url, Status) {
    var doc = document,
...
function exec(scrawlObj) {
    if (scrawlObj.isScrawl) {
        ...
        if (!!base64) {
            ...
            formData.append("clientid", editor.getOpt("clientid"));
            formData.append("filetree", editor.getOpt("imagetree"));
            formData.append("referkey", editor.getOpt("referkey"));
            formData.append("imageName",convertBase64UrlToBlob(base64),Date.now()+'.jpg');
            var actionUrl = editor.getActionUrl(editor.getOpt('ActionName'));
            $.ajax({
                url:actionUrl,
                dataType:'json',
                type:'POST',
                ...
                success: function(data) {
                    if (!scrawlObj.isCancelScrawl) {
                        if (data.Status == 1) {
                            var imgObj = {},
                                url = editor.options.UrlPrefix + data.Item.id;
                            imgObj.src = url;
                            imgObj._src = url;
                            imgObj.alt = data.Item.FileName || '';
                            imgObj.title = data.Item.FileName || '';
