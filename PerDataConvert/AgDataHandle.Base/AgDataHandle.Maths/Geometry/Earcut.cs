...
namespace AgDataHandle.Maths.Geometry
{
    ...
    public class Earcut
    {
        public static List<int> Tessellate(List<Polygon> data)
        {
            //var hasHoles = false;// holeIndices.Count > 0;
            if (data.Count == 0)
                return null;
            Node outerNode = LinkedList(data[0], 0, data[0].Count - 1, 0, true);
            if (outerNode == null)
            {
                return null;
            }
            ...
            double invSize = default(double);

            if (data.Count > 1)
            {
                outerNode = EliminateHoles(data, outerNode);
            }
        ...
        static Node LinkedList(Polygon data, int start, int end,int lastIndex, bool clockwise)
        {
            ...
            if (clockwise == (SignedArea(data, start, end) > 0))
            {
                ...
                {
                    last = InsertNode(i+ lastIndex, data[i].X, data[i].Y, last);
                }
            }
            else
            {
        ...
        static Node EliminateHoles(List<Polygon> data, Node outerNode)
        {
            int len = data.Count;
            List<Node> queue = new List<Node>();
            int lastIndex = data[0].Count - 1;
            for (int i = 1; i < len; i++)
            {
                Node list = LinkedList(data[i], 0, data[i].Count - 1, lastIndex, false);
                if (list == list.next)
                {
                    list.steiner = true;
                }
        ...
        static Node FindHoleBridge(Node hole, Node outerNode)
        {
            ...
            do
            {
                ...
                {
                    double x = p.x + (hole.y - p.y) * (p.next.x - p.x) / (p.next.y - p.y);
                    if (x <= hole.x && x > qx)
                    {
                        qx = x;
                        if (x == hole.x)
            ...
            while (p != stop)
            {
                if (hole.x >= p.x && p.x >= mx && hole.x != p.x && PointInTriangle(hole.y < my ? hole.x : qx, hole.y, mx, my, hole.y < my ? qx : hole.x, hole.y, p.x, p.y))
                {

                    tan = Math.Abs(hole.y - p.y) / (hole.x - p.x); // tangential
        ...
        static double SignedArea(Polygon data, int start, int end)
        {
        ...
        //public static double Deviation(List<PointF> data, List<int> holeIndices, List<int> triangles)
        //{
        ...
        //            (data[a].X - data[b].X) * (data[c].Y - data[a].Y));
        //    }
