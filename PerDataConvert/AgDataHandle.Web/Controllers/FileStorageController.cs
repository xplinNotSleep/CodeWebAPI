...
namespace AgDataHandle.Web.Controllers
{
    ...
    public class FileStorageController : ApiControllerBase
    {
        ...
        public FileStorageController(FileStorageService OFileStorageService)
        {
            ...
            UploadPath = Toolset.AppConfigHelper["UploadPath"]; //Path.Combine(Directory.GetCurrentDirectory(), "upload");
        }
        ...
        /// <returns></returns>
        [HttpPost("Upload")]
        public async Task<BaseMessage<UploadResultData>> Upload([FromForm] FileStorageUploadRequestParam model)
        {
            try
            {
                ...
                using (var stream = fileForm.OpenReadStream())
                {
                    ...
                    #endregion
                    string secret = PasswordHelper.MD5(System.Text.Encoding.Default.GetString(buffer) + buffer.Length + extName);
                    var path = "";
                    ...
                    {
                        #region 为文件分配路径
                        var fileTreePath = UploadPath;// Path.Combine(UploadPath, model.FileTree);
                        ...
                        path = Path.Combine(fileTreePath, Path.GetFileNameWithoutExtension(fileForm.FileName) +"_"+ DateTime.Now.ToString("yyyyMMddHHmmss") + Path.GetExtension(fileForm.FileName));
                        ...
                        uploadresult.uploadFilePath= file.FilePath;
                    }
                }
